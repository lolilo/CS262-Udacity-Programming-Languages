...
# example, prefixes("cat") == [ "c", "ca", "cat" ]. 
def prefixes(X):
    return [ X[:char + 1] for char in range(len(X))]

# returns the length of the longest common substring of X and Y by considering 
# all prefixes x of X and all prefixes y of Y and returning the biggest csuffix(x,y)
# encountered. 
def lsubstring(X,Y):
    l = [csuffix(x,y) for x in prefixes(X) for y in prefixes(Y)]
    return max(l)
