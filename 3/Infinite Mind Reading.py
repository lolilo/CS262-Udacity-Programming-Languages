...
# returns True if the grammar
# accepts an infinite number of strings
def cfginfinite(grammar): 
      # construct list of left-hand side symbols
      for Q in [ rule[0] for rule in grammar ]:
      # see if Q can be rewritten to x Q y where | x + y | > 0
            def helper(current, visited, sizexy):
                  if current in visited: # we have looped
                        return sizexy > 0 # returns True/False, which correponds to whether grammar is infinite
                  else: 
                        new_visited = visited + [current] # update visited list
                        for rhs in [ rule[1] for rule in grammar if rule[0] == current ]:
                              for symbol in rhs:
                                    # minus one from sizexy to exclude what we are recurring on (symbol Q)
                                    if helper(symbol, new_visited, sizexy + len(rhs) - 1): 
                                          return True
                        return False # if helper function never returns True, language is not infinite
