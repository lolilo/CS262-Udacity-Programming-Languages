...
def addresses(haystack): 
    regexp1 = r'[a-zA-Z]+\@[a-zA-Z]+(?:\.[a-zA-Z]+)+'
    regexp2 = r'NOSPAM'
    addresses = re.findall(regexp1, haystack)
    print addresses
    for i in range(len(addresses)):
        addresses[i] = re.sub(regexp2, '', addresses[i])
    return addresses

    # one liner. maybe not as efficient, since it iterates through text 
    # to replace all the NOSPAM. Also, \w isn't exactly [a-zA-Z]
    # return re.findall( r"\w+@\w+(?:\.\w+)+", re.sub(r"NOSPAM", "", haystack) )
